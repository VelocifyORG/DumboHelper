...
tasks {
    assemble {
        if (providers.gradleProperty("autoFormat").orElse("false").map { it.toBoolean() }.get()) {
    ...
    create("setup", Copy::class) {
        from(rootProject.rootDir.resolve(".spotless/scripts"), "pre-commit")
